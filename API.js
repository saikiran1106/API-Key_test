...
app.post("/login", async (req, res) => {
  ...
  try {
    ...
    const user_mobile = await details.findOne(query2);
...
app.post("/monthly-consumption", async (req, res) => {
  ...
  try {
    ...
    const query = ({ mobile: mobile } , { name: 1, monthly_consumption: 1 });
    const user = await details.findOne({ mobile: mobile } , { name: 1, monthly_consumption: 1 , otp: 1 });
    const otp = user.otp;
...
app.post("/login-test", async (req, res) => {
  ...
  res.redirect(`http://localhost:3000/dashboard/index.html?mobile=${encodeURIComponent(mobile)}`);
}
  } 
